---
name: release
description: Cut a GitHub release of mad-eye — inferred bump, signed + notarized universal macOS DMG, tag, GitHub release with changelog, and Homebrew cask bump.
...
Distribution is **signed with a Developer ID certificate and notarized by Apple** (ADR 0004 + 0005): `tauri.conf.json`'s `bundle.macOS.signingIdentity` signs the bundle, and `scripts/release-mac.sh` builds the universal DMG, submits it to `notarytool` (via the `mad-eye-notary` Keychain credential profile), and staples the ticket — so Gatekeeper opens downloads cleanly with **no `xattr` workaround**. This also keeps the Keychain "Always Allow" ACL grant stable across updates (the signing identity is what the ACL binds to). mad-eye uses `macOSPrivateApi` (transparent + vibrancy Popover), which bars the Mac App Store, so the Homebrew cask is the distribution channel. The app version lives in `src-tauri/tauri.conf.json` (`CFBundleVersion` derives from it); `package.json` + `src-tauri/Cargo.toml` mirror it in lockstep.

**One-time setup** (before the FIRST release, else the gates fail): a GitHub remote exists (`git remote add origin …` + an initial push); both universal Rust targets installed (`rustup target add aarch64-apple-darwin x86_64-apple-darwin`); `gh` authenticated; a **Developer ID Application** certificate in the login Keychain and a `notarytool` credential profile named `mad-eye-notary` (`xcrun notarytool store-credentials mad-eye-notary --apple-id <id> --team-id <team>`). The Homebrew tap (`kvnwolf/homebrew-tap`) and its cask are scaffolded by Step 5 on the first release if missing.
...
5. **Signing + notarization ready**: `security find-identity -v -p codesigning` lists a `Developer ID Application` identity, AND `xcrun notarytool history --keychain-profile mad-eye-notary` succeeds (the credential profile exists). Missing either → STOP and point to the one-time setup.
6. **Fresh `node_modules`**: `bun run build` (vite) succeeds. The main checkout's `node_modules` can go stale between releases — if `@kvnwolf/dobby` is a dangling symlink to a removed worktree (e.g. a deleted Conductor workspace), vite fails with `Tsconfig not found @kvnwolf/dobby/tsconfig/vite`. Fix: `rm -rf node_modules && bun install`, then re-check. (This is a gate, not the real build — Step 3 rebuilds under `tauri build`.)
7. **CI** (only if `.github/workflows/` exists): `gh run list --branch main --limit 1` → completed/success (watch it to completion if still running). No workflows yet → skip.
...
## Step 3: Build, sign, and notarize the universal DMG

- Run **`./scripts/release-mac.sh`** — it builds the Universal binary (Apple Silicon + Intel, running `beforeBuildCommand`/`vite build` first), signs it with the Developer ID identity from `tauri.conf.json`, submits the DMG to `notarytool --keychain-profile mad-eye-notary --wait`, and staples the ticket. Do NOT run it through a pipe (`| tail`) — a pipe masks the script's exit code behind the last stage's, hiding a build failure. Capture the exit code directly.
- Locate the artifact: `DMG=$(ls src-tauri/target/universal-apple-darwin/release/bundle/dmg/mad-eye_<V>_universal.dmg)` — match the version explicitly, so a stale older DMG in `target/` is never mistaken for this build.
- Gate: the script exits 0 AND its notarization line reads `status: Accepted`; exactly ONE `.dmg` at the versioned path; the built app's version matches —
  `/usr/libexec/PlistBuddy -c "Print :CFBundleShortVersionString" "src-tauri/target/universal-apple-darwin/release/bundle/macos/mad-eye.app/Contents/Info.plist"` → `<V>`; and `spctl -a -t open --context context:primary-signature -vv "$DMG"` → `accepted` / `source=Notarized Developer ID`.
...
- Optional local smoke (skip if the user doesn't want it live-installed on this machine): `brew install --cask --force kvnwolf/tap/mad-eye`, then launch and confirm the Eye appears in the menubar. No `xattr` needed — the DMG is notarized, so Gatekeeper opens it cleanly. On first launch click **"Always Allow"** on the Keychain prompt.
- Report to the user: version published, inferred bump + reasoning, DMG path + `sha256`, notarization verdict, tag + GitHub release URL, cask commit URL, and the `brew install --cask kvnwolf/tap/mad-eye` command.

## Acceptance checklist

- [ ] All Step-1 preflight gates passed (main checkout + branch, clean + current, gh auth + remote, both universal targets, Developer ID identity + `mad-eye-notary` profile, fresh `node_modules`/vite build, CI if present)
...
- [ ] Universal DMG built via `scripts/release-mac.sh` (not piped), signed + notarized (`status: Accepted`), stapled, `spctl` accepts it as Notarized Developer ID; exactly one artifact at the versioned path; the built `.app`'s `CFBundleShortVersionString` equals `<V>`
...
- [ ] Reported: version, bump reasoning, DMG + sha, notarization verdict, release URL, cask URL, `brew install --cask` command
