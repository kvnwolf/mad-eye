...
/** The app's version stamp (Rust `app_version` command): shown in the footer,
 * with a DEV badge when the binary is a debug/dev build. */
...
export interface PopoverOptions {
  ...
  refreshing?: boolean;
  /** The footer's "v0.1.1" stamp (+ DEV badge); omitted → no stamp. */
  version?: VersionInfo;
}
...
function renderControls(
  ...
  if (opts.version) {
    footer.append(el("span", "popover__version", `v${opts.version.version}`));
    if (opts.version.dev) {
      footer.append(el("span", "popover__dev", "DEV"));
    }
  }
