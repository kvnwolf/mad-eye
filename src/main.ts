...
import { renderPopover, type VersionInfo } from "./popover/render";
import {
  mockMoodFor,
  mockSnapshot,
  type Snapshot,
...
if (runtime.__TAURI__ || runtime.__TAURI_INTERNALS__) {
  ...
  // The footer's version stamp — fetched once at boot (it can't change while
  // the app runs) and threaded into every paint.
  ...
  const paint = (snap: Snapshot): void => {
    current = snap;
    renderPopover(root, snap, { refreshing, version });
    ...
    if (height > 0) {
      getCurrentWindow()
        .setSize(new LogicalSize(POPOVER_WIDTH, height))
        .catch(() => undefined);
    }
  };
  ...
  const startRefresh = (): void => {
    ...
    invoke("refresh_now_cmd").catch(() => undefined);
  ...
  const selectDriver = (control: HTMLElement): void => {
    ...
    if (name !== undefined) {
      invoke("set_driver", { name }).catch(() => undefined);
    }
  };
  ...
  version = await invoke<VersionInfo>("app_version");
  ...
  // The dev-URL footer shows a mock stamp (this path never boots Tauri, so
  // there is no real version to ask for) — dev-flagged, like the mock data.
  const mockOpts = { version: { dev: true, version: "0.0.0" } };
  let snap = mockSnapshot(coerceStatus(requested));
  renderPopover(root, snap, mockOpts);
